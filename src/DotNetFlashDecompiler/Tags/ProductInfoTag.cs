    ...
    uint BuildLow, uint BuildHigh, DateTime CompilationDate) : TagItem, IBufferReadable<TagItem>
{
    ...
    {
        value = default;
        if (!reader.TryReadBigEndian(out uint product)) return false;
        if (!reader.TryReadBigEndian(out uint edition)) return false;
        if (!reader.TryRead(out byte majorVersion)) return false;
        if (!reader.TryRead(out byte minorVersion)) return false;
        if (!reader.TryReadBigEndian(out uint buildLow)) return false;
        if (!reader.TryReadBigEndian(out uint buildHigh)) return false;
        if (!reader.TryReadBigEndian(out ulong compilationDate)) return false;
